...
$FileName = "";
  /*SELECT ar.*, ra.*
	...
	$sql = mysqli_query($connection, "
		 SELECT ar.*, ra.*, ev.*
		FROM paper as ar
		LEFT JOIN (paper_review as ra, event as ev)
		ON ar.paper_id = ra.paper_id
		AND ar.event_id = ev.event_id
		WHERE ra.review_status != 0 
		");
	     if ($sql)
		if (mysqli_num_rows($sql) > 0) {
			$message = "<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'> 
			<thead><tr><th>SN</th><th>Article Title</th> <th>Article</th> <th>Journal/Conference</th><th>Review Status</th> <th>Comment</th> <th>Publish</th></tr></thead>
			<tbody>";
		              ...
		              if(!empty($row['abstract']))
						...
						/*$body2 =" <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#exampleModalLond".$sn."'>
								...
								$assign_form = "<a href='assign_editor.php?paper=".$row['paper_id']."'><span>Assign Editor</span></a>";			
											."";*/
											$publish_form = "<a href='publish.php?paper=".$row['paper_id']."'><span>Publish Paper</span></a>";
								if ($row['result'] == 0)
                    	...
                    	$message .= "<tr><td>".$sn."</td><td>".$row['title']."</td><td> ". $body."  /   ". $body2."</td><td>".$row['event_name']."</td><td>".$review_status."</td><td>".$row['comment']."</td><td>". $publish_form ."</td></tr>";
									}
						$message .= "<tbody></table>";
					}
				    else
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Publish Reviewed Article</h6>
            </div>
            <div class="card-body">
