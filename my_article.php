...
$sql = mysqli_query($connection, "
    ...
    while($row = mysqli_fetch_assoc($sql)) {
		              ...
		              if(!empty($row['abstract']))
						...
						/*$body2 =" <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#exampleModalLond".$sn."'>
							 ...
							 $message = "<h3>Article Details</h3><p> <strong>Title : </strong>".$row['title']."</p><p> Article Body  : ".$body." / ".$body2."</p>";							 
	...
	$sql = mysqli_query($connection, "
		...
		");
	     if ($sql)
		if (mysqli_num_rows($sql) > 0) {
			$message .= " <h3>Review Information</h3><table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'> 
			<thead><tr><th>SN</th> <th>Review Status</th> <th>Comment</th></tr></thead>
			<tbody>";
    ...
    while($row = mysqli_fetch_assoc($sql)) {
		              ...
		              if(!empty($row['abstract']))
                    	...
                    	$message .= "<tr><td>".$sn."</td><td>".$review_status."</td><td>".$row['comment']."</td></tr>";
									}
						$message .= "<tbody></table>";
					}
				    else
					$message .= "<strong> Article yet to be reviewed</strong>";
					else {
